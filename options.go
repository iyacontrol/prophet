...
import (
	...
	"github.com/iyacontrol/prophet/pkg/net"
)

const (
	defaultLeaderElection          = true
	defaultLeaderElectionID        = "prophet-controller-leader"
...
func (options *Options) BindFlags(fs *pflag.FlagSet) {
	fs.BoolVar(&options.ShowVersion, "version", false,
		`Show release information about the k8s prophet controller and exit.`)
	...
	fs.BoolVar(&options.LeaderElection, "election", defaultLeaderElection,
		`Whether we do leader election for prophet controller`)
	fs.StringVar(&options.LeaderElectionID, "election-id", defaultLeaderElectionID,
		`Namespace of leader-election configmap for prophet controller`)
	fs.StringVar(&options.LeaderElectionNamespace, "election-namespace", defaultLeaderElectionNamespace,
		`Namespace of leader-election configmap for prophet controller. If unspecified, the namespace of this controller pod will be used`)
